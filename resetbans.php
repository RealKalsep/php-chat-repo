...
<?php
$blocked = array("<style>", "<script>"); // obsolete, remove. headache.
...
// if admin, clear bans, if not, redirect
if(strpos($idlist, $crypt) !== false) {
    $file = fopen("dEz6tWULWp.txt", "w") or die("CANNOT FIND FILE. ABORTING");
    fwrite($file, ""); #clearing bans
    fclose($file);
} else {
